...
### Как добавить страницу или раздел {#pagecms-create-entry}

Краткий порядок действий (первые шаги уже подробно разобраны выше).

**Шаг 1 — Ветка редактирования.** Убедись, что активна **`cms-edits`** (или другая согласованная рабочая ветка), не **`main`**. Иначе сохранения попадут не туда или будут отклонены защитой GitHub. См. раздел [«Ветка для редактирования»](#pagecms-editing-branch).

**Шаг 2 — Коллекция языка.** В блоке **Content** левой панели выбери, **в каком языке** создаёшь или редактируешь материал:

- **Документация RU** — файлы попадут в `docs/ru/`.
- **Documentation EN** — файлы попадут в `docs/en/`.

Активный пункт подсвечивается; справа откроется список записей этой коллекции (например **Заголовок** / **Title** и кнопки **Add an entry**, **Edit** и т.д.).

![Шаг 2 — выбрана коллекция «Документация RU», ветка `cms-edits`](/images/pagecms-admin-step-02-language-collection.png)

**Шаг 3 — Как именно добавить материал.** Над списком записей коллекции (рядом с поиском) есть **два разных действия**:

![Шаг 3 — два варианта: отдельная страница и раздел страниц](/images/pagecms-admin-two-ways-create.png)

| Действие | Что это даёт на сайте и в Git |
| ------- | ----------------------------- |
| **Add an entry** (зелёная кнопка) | **Отдельная страница** на текущем уровне: один файл `{заголовок}.md` в `docs/ru/` или `docs/en/`, без отдельной папки-раздела. |
| **Кнопка с иконкой папки и «+»** | **Раздел** (ветка дерева в коллекции): в репозитории создаётся подпапка; у узла раздела по конфигу `.pages.yml` задаётся страница **`index.md`** ([зачем она нужна](#pagecms-section-index-required)); внутри можно добавить вложенные страницы. |

Дальше в этом разделе будут добавлены **пошаговые инструкции для каждого варианта** по отдельности: отдельная страница и раздел с подстраницами.
